...
export SUPABASE_HOST="${SUPABASE_HOST:-localhost}" SUPABASE_PORT="${SUPABASE_PORT:-5432}"
...
psql -h "$SUPABASE_HOST" -p "$SUPABASE_PORT" -d presupuestos_smoke -c "TRUNCATE presupuestos.ced_presupuestos;"
...
psql -h "$SUPABASE_HOST" -p "$SUPABASE_PORT" -d presupuestos_smoke -c "SELECT capa, COUNT(*) filas, COUNT(DISTINCT ccaa) ccaas, MIN(periodo) anio_min, MAX(periodo) anio_max FROM presupuestos.ced_presupuestos GROUP BY capa;"

# 3b) Celdas autonómicas faltantes vs grid 17×(2015-2026) — deben ser solo cym 2015 y mur 2026
psql -h "$SUPABASE_HOST" -p "$SUPABASE_PORT" -d presupuestos_smoke -tAc "WITH grid AS (SELECT c.ccaa,g.periodo FROM (SELECT DISTINCT ccaa FROM presupuestos.ced_presupuestos WHERE capa='autonomica') c CROSS JOIN generate_series(2015,2026) g(periodo)), have AS (SELECT ccaa,periodo FROM presupuestos.ced_presupuestos WHERE capa='autonomica') SELECT 'FALTA: '||grid.ccaa||' '||grid.periodo FROM grid LEFT JOIN have USING(ccaa,periodo) WHERE have.ccaa IS NULL ORDER BY 1;"

# 4) Spot-check de sanidad por CCAA-año (€ constantes en DB)
psql -h "$SUPABASE_HOST" -p "$SUPABASE_PORT" -d presupuestos_smoke -c "SELECT ccaa, periodo, capa, ROUND((COALESCE(imp_sanidad,0)/1e9)::numeric,2) AS sanidad_b FROM presupuestos.ced_presupuestos WHERE capa='autonomica' ORDER BY ccaa, periodo;"
