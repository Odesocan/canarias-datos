...
**Fecha:** 2026-06-26 (Noche 30) · **Fuente:** `outputs/smoke_regresion_py.csv` (catálogo autoritativo)

Salud verificada esta noche en aislado (sin R/psql en el sandbox; solo `python3`): re-extracción multi-motor OK — `pvc/2025`=122, `and/2015`=114, `gal/2025`=47, `mur/2025`=106, `lar/2025`=56, `cym/2026`=103, `ext/2025`=74 filas, todas cuadran con el catálogo. Sin cambios de cobertura respecto a la Noche 29 (no hay raws nuevos descargables; `web_fetch` sigue restringido al *provenance set*, re-confirmado con el dataset Xunta 0665/2026 → "URL not in provenance set"). `can/2025` y `nav/2026` no se pudieron re-leer por el fallo intermitente de mount `EDEADLK` (el `cp` del propio fichero falla a nivel de filesystem; no es regresión de código).
...
**Total: 91 ejercicios-año en VERDE · 17/17 CCAA · 1 ERROR (ara/2016).** Matriz regenerada esta noche desde el catálogo vivo (`tools/smoke_regresion_py.py` → `outputs/smoke_regresion_py.csv`, 92 filas); idéntica a la Noche 29 → catálogo estable, sin regresiones.
...
|---|---|---|---|
| ast/2022 | `tomo_I.pdf` 19.8 MB | Ley 6/2021 BOPA (articulado) → extracción da **0 filas** | Falta tomo "Distribución del gasto por programas" |
...
Todos requieren descargar el raw correcto. En el sandbox `web_fetch` está restringido al *provenance set* (re-probado esta noche) y no se permite `curl`/`wget`/`requests`. Sin descargas nuevas posibles aquí → pendiente de un run con red estable fuera del sandbox (o de que el usuario adjunte los raws en el mensaje).
